...
__call__ = ['Accuracy', 'AUC', 'F1Score', 'EntityScore',
            'ClassReport', 'MultiLabelReport', 'AccuracyThresh']
...
class AccuracyThresh(Metric):
    ...
    def value(self):
        ...
        acc = np.mean(((self.y_pred > self.thresh) ==
                       self.y_true.bool()).float().cpu().numpy(), axis=1).sum()
...
class AUC(Metric):
    ...
    def value(self):
        ...
        auc = roc_auc_score(y_score=self.y_prob,
                            y_true=self.y_true, average=self.average)
        return auc
...
class F1Score(Metric):
    ...
    def value(self):
        '''
         计算指标得分
         '''
        f1 = f1_score(y_true=self.y_true, y_pred=self.y_pred,
...
class MultiLabelReport(Metric):
    ...
    def value(self):
        ...
        for i, label in self.id2label.items():
            try:
                auc = roc_auc_score(
                    y_score=self.y_prob[:, i], y_true=self.y_true[:, i])
                print(f"label:{label} - auc: {auc:.4f}")
            except ValueError as e:
                print(f"label:{label} - auc: can not calculate")
